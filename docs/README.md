...
Un sistema completo de comunicación segura cliente-servidor con cifrado híbrido (RSA + AES), SSL/TLS y **firma digital de documentos** (v6.0). Implementado en Python con arquitectura asíncrona para máximo rendimiento. Configuración mediante variables de entorno sin valores hardcodeados.

> **📖 Para usuarios no técnicos:** Lee la sección [**"Guía para Usuarios No Técnicos"**](#-guía-para-usuarios-no-técnicos) más abajo.  
> **⚙️ Para ingenieros/desarrolladores:** Continúa con la documentación técnica completa.
...
- `PyPDF2>=3.0.0`: Manejo de archivos PDF (v6.0)
- `aiohttp>=3.9.0`: Servidor HTTP asíncrono (v6.0)
...
- Genera claves de firma con: `python -c "from digital_signature import generate_signing_key_pair; generate_signing_key_pair()"`
...
### Probar Firma Digital (v6.0)
```bash
# 1. Generar par de claves para firma
python -c "from digital_signature import generate_signing_key_pair; generate_signing_key_pair()"

# 2. Iniciar servidor de archivos
python file_server.py

# 3. En otra terminal, subir un archivo (usando curl o Postman)
...
```
chat/
├── server.py              # Servidor TCP/SSL asíncrono con cifrado híbrido
├── client.py              # Cliente TCP/SSL asíncrono con cifrado híbrido
├── file_server.py         # Servidor HTTP para subida y firma de archivos (v6.0)
├── digital_signature.py   # Módulo de firma digital (v6.0)
...
├── README.md             # Este archivo (documentación técnica y no técnica)
├── README.txt            # Historial detallado de cambios
├── CONTROL_CAMBIOS.txt   # Documento de control de cambios
├── RESUMEN_EJECUTIVO.md  # Resumen ejecutivo (v6.0)
├── PROJECT_CHARTER.md    # Carta del proyecto (v6.0)
├── PRESUPUESTOS.md       # Análisis de presupuestos (v6.0)
├── certificates/         # Directorio para certificados SSL
│   ├── server.crt        # Certificado del servidor
│   ├── server.key        # Clave privada del servidor
│   └── ca.crt            # Certificado CA (self-signed)
├── signing_keys/         # Directorio para claves de firma (v6.0)
│   ├── signing_private_key.pem
│   └── signing_public_key.pem
├── uploads/              # Directorio para archivos subidos (v6.0)
├── signatures/           # Directorio para firmas digitales (v6.0)
...
### Versión 6.0 - Firma Digital y Servidor de Archivos (Actual)
- ✅ Módulo de firma digital para archivos (TXT, PDF, ZIP)
- ✅ Servidor HTTP asíncrono para subida y firma de archivos
- ✅ Generación de pares de claves para firma digital
- ✅ Verificación de firmas digitales
- ✅ Documentación ejecutiva (Resumen, Project Charter, Presupuestos)
- ✅ README con secciones para usuarios no técnicos e ingenieros
- ✅ Preparado para despliegue en AWS o máquinas virtuales
...
**Última actualización: 2025-11-19 (Versión 6.0)**
...
| digital_signature.py | `258fc85d3be210d0f324901e058b19e2`    | 2025-11-19      | 6.0     |
| file_server.py   | `571bba308266bcb4a1e3bc0c2da34da7`    | 2025-11-19      | 6.0     |
